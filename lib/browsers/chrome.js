...
chrome.setUpEvents = async function (tab) {
  ...
  Network.requestWillBeSent((params) => {
    ...
    const staticFileRegex =
      /\.woff2|\.ttf|\.eot|\.otf|\.woff|\.png|\.gif|\.tiff|\.pdf|\.jpg|\.jpeg|\.ico|\.svg|\.webp/;

    const excludePaths = ['featureassets.org/v1/initialize'];

    if (
      sentryUrlRegex.test(params?.request?.url) ||
      staticFileRegex.test(params?.request?.url) ||
      excludePaths.some((path) => params?.request?.url.includes(path))
    ) {
      return;
    }
  ...
  Network.dataReceived(({ requestId, dataLength }) => {
    ...
    // prevent exit becaouse download big file.
    tab.prerender.lastRequestReceivedAt = new Date().getTime();
    ...
    tab.Runtime.evaluate({
      ...
      .then((result) => {
        ...
        console.log(
          ...
          doneLoading,
          prerenderReady,
          timeSpentAfterFirstPrerenderReady,
          prerenderReadyDelay,
        );
        resolve(
    ...
    tab.Runtime.evaluate({
      ...
      .then((resp) => {
        ...
        if (tab.prerender.content === undefined) {
          tab.prerender.statusCode = 504;
          tab.prerender.statusCodeReason =
            'unable to evaluate getHtmlFunctionText';
        }
