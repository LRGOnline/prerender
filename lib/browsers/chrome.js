...
chrome.setUpEvents = async function (tab) {
  ...
  Network.requestWillBeSent((params) => {
    const sentryUrlRegex = /ingest\.sentry\.io/;
    // prevent static assets from being requested, so as to reduce the load on Varnish
    const staticFileRegex = /\.woff2|\.ttf|\.eot|\.otf|\.woff|\.png|\.gif|\.tiff|\.pdf|\.jpg|\.jpeg|\.ico|\.svg|\.webp/;
    
    if (sentryUrlRegex.test(params?.request?.url) ||staticFileRegex.test(params?.request?.url)) {
        return;
    }    
    ...
    tab.Runtime.evaluate({
      ...
      .then((result) => {
        ...
        const timeSpentAfterFirstPrerenderReady =
          ...
          tab.Runtime.evaluate({
            expression: "({title: document.querySelector('meta[name=\"title\"]')?.innerText, description: document.querySelector('meta[name=\"description\"]')?.content})",
          })
            .then((metaResult) => {
              const metaTitle = metaResult.result.value.title;
              const metaDescription = metaResult.result.value.description;
  
              if (!metaTitle || !metaDescription) {
                util.log(`Cant find the meta tags on the page url=${url}`);
                return resolve(false);
              }
            ...
            .catch((err) => {
              util.log(`unable to evaluate meta tags on the page err=${err}, url=${url}`);
              tab.prerender.statusCode = 504;
              tab.prerender.statusCodeReason =
                'unable to evaluate meta tags on the page';
